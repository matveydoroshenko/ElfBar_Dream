...
async def admin_menu(call: CallbackQuery):
    await call.message.edit_text(text=f"Привет!", reply_markup=sub_admin_keyboard)


async def add_delete_product(call: CallbackQuery):
    await call.message.edit_text(text=f"Привет!",
...
async def back_to_admin(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(text=f"Привет!", reply_markup=admin_keyboard)
    await state.finish()
...
async def back_to_edit_text(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(text=f"Привет!", reply_markup=texts())
    await state.finish()


async def back_to_add_product(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(text=f"Привет!", reply_markup=add_delete_admin_keyboard)
    await state.finish()


async def back_to_user(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(text=f"Привет!", reply_markup=main_keyboard())
    await state.finish()
...
async def back_to_list(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(text=f"Привет!", reply_markup=products_list(puffs))
    await state.finish()


async def back_to_categories(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await call.message.answer(text="Список товаров:", reply_markup=categories())
    await state.finish()
...
async def products(call: CallbackQuery):
    await call.message.edit_text(text="Список товаров:", reply_markup=categories())
...
async def order_callback_buttons(call: CallbackQuery, callback_data: dict):
    ...
    user_text = db.select_text(position="/start")[0]
    await call.message.answer(user_text, reply_markup=main_keyboard())
...
async def delete_callback_buttons(call: CallbackQuery, callback_data: dict):
    ...
    await call.message.answer("Привет!", reply_markup=admin_keyboard)


async def puffs_product(call: CallbackQuery, callback_data: dict):
    puffs_number = callback_data.get("puffs_number")
    await call.message.edit_text(text="Выбери товар:", reply_markup=products_list(puffs_number))
